...
// import { stringify } from 'postcss';
  ...
  resdata.map(function(val) {
    ...
    categorywisequestions.value = result.map(x => {
       ...
       return {'qid' : x.question_id, 'question' : x.question , 'options' : convertgrpoption(x.options,x.answer) , 'answer' : x.answer, 'category_id' : x.category_id ,'company_id' : x.company_id}
       // let obj = {}
...
// group.value = answer
 ...
 return output
    ...
    const getcategorydetails = () => {
      api
      .get(`analytic/getcategorydetails/${categoryid.value}`, {
          headers: {
            Authorization: "Bearer " + token.value,
          },
        })
        .then(async (res) => { console.log(res) 
          let resdata = res.data.data
          resdata.map(val => {
           categoryname.value = val.category
          })
        })
    }
    ...
    const savequestions = () => {
      console.log(question.value,selectcategory.value,group.value)

      api.post('analytic/insertqstn', {question :question.value,options:'[]',company_id: admin.value.company_id,category_id: selectcategory.value,questiontype : defaultquestiontype.value,answer : ''},
    {
  headers: {
    Authorization: 'Bearer ' + token.value
  }
...
<template>
  <q-page
    ...
    <q-btn round color="primary" @click="alert" icon="alarm" />
      ...
      >
        {{ categoryname }}<span class="button" style="padding-right: 10px"
          ><q-btn outline rounded color="green" label="Preview Questions" no-caps/><q-btn
          ...
          <q-tab-panel name="addquestions">
            <div class="row items-start">
              <div class="q-px-md" style="max-width:420px">
                 ...
                 > <q-card  class="q-pa-sm q-gutter-sm " style="border-radius:10px" flat  v-for="(qid,question) in categorywisequestions" :key="question" :name="question" >
    ...
    <q-card v-if="switchforedit" class="my-card" style="width: 600px;padding-left:10px" flat>
                  <div class="text-h6">Add Questions <span class="button q-gutter-sm"><q-btn outline rounded color="green" label="Add new Category" @click="show_dialog2 = !show_dialog2" no-caps></q-btn></span></div>
                ...
                <q-card v-else class="my-card" style="width: 600px;padding-left:10px" flat >
                  <div class="text-h6 row">Edit Questions <span class="button q-gutter-sm"><q-btn outline rounded color="green" no-caps label="add new question" @click="swapeditquestion()"></q-btn><q-btn  outline rounded color="red" no-caps label="delete" ></q-btn></span></div>
                  <br />
                  <div class=" q-gutter-sm">
